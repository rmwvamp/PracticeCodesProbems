...
void solve()
{
    ...
    sci(n);
    vector<ll> arr(n), p(n), q(n);
    map<ll, vector<ll>> mmp;
    vector<ll> missing, present2;
    for (int i = 0; i < n; i++)
    {
        cin >> arr[i];
    ...
    for (int i = 1; i <= n; i++)
    {
        if (mmp.find(i) != mmp.end())
        {
            ...
            else if (mmp[i].size() == 2)
            {
                present2.push_back(i);
            }
        }
        else
            missing.push_back(i);
    ...
    sort(all(missing), greater<ll>());

    sort(all(present2), greater<ll>());
    ll mm = 0;

    // dbgm(missing, present2);
    vector<bool> check(n);
    for (int i = 0; i < present2.size(); i++)
    {
        ...
        if (x < present2[i])
        {
            ...
            p[mmp[present2[i]][0]] = present2[i];
            q[mmp[present2[i]][0]] = x;
            check[mmp[present2[i]][0]] = true;
            p[mmp[present2[i]][1]] = x;
            q[mmp[present2[i]][1]] = present2[i];
            check[mmp[present2[i]][1]] = true;
        }
        else
        {
            cout << "NO" << endl;
