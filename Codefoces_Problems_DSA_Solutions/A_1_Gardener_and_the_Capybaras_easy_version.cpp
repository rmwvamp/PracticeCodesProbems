...
//     return memo[i][j] = ans;
// }
map<string, ll> printSubstrings(string str)
{
...
void solve()
{
    ...
    for (auto x : mmpa)
    {
        ...
        for (auto y : mmpb)
        {
            ...
            {
                string sc = z.first;
                if (sa.size() + sb.size() + sc.size() == s.size() && (sa + sb + sc) == s)
                {
                    if (sb.compare(sa) >= 0 && sb.compare(sc) >= 0 || (sa.compare(sb) >= 0 && sc.compare(sb) >= 0))
                    {
                        cout << sa << " " << sb << " " << sc << endl;
                        return;
                    }
