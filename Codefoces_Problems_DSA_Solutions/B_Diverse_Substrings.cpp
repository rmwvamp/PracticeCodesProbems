...
//     return memo[i][j] = ans;
// }
void insert(int x, vector<int> &FrequencyofElements, int &DistinctElementsCount, int &maxfrequecy, int &index)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        maxfrequecy = max(maxfrequecy, FrequencyofElements[i]);
        index = i;
    }
}
void remove(int x, vector<int> &FrequencyofElements, int &DistinctElementsCount, int &maxfrequecy, int &index)
{
...
void solve()
{
    ...
    vector<int> FrequencyofElements(10, 0);
    int maxfrequecy = 0;
    int index = -1;
    ...
    {
        // maxfrequecy=-1;
        while (head + 1 < n && ((maxfrequecy < DistinctElementsCount) || (maxfrequecy == DistinctElementsCount && FrequencyofElements[arr[head + 1] - '0'] < DistinctElementsCount) || (head - tail < 0)))
        {
            head++;
            insert(arr[head] - '0', FrequencyofElements, DistinctElementsCount, maxfrequecy, index);
        }
        dbgm(head);
        ...
        if (head >= tail)
        {
            remove(arr[tail] - '0', FrequencyofElements, DistinctElementsCount, maxfrequecy, index);
            tail++;
        }
        else
        {
...
void AfterVideo_Solve()
{

    sci(n);
    scs(arr);
    // max string length would be 100
    vector<ll> FrequencyCount(10);
    ll DistinctElementsCount = 0, MaxFrequecyCount = 0, ans = 0;
    for (int i = 0; i < n; i++)
    {
        DistinctElementsCount = 0, MaxFrequecyCount = 0;
        FrequencyCount.assign(10, 0);
        for (int j = i; j <= i + 99 && j < n; j++)
        {
            /* code */
            FrequencyCount[arr[j] - '0']++;
            if (FrequencyCount[arr[j] - '0'] == 1)
                DistinctElementsCount++;
            MaxFrequecyCount = max(MaxFrequecyCount, FrequencyCount[arr[j] - '0']);
            if (MaxFrequecyCount <= DistinctElementsCount)
                ans++;
        }
    }
    cout << ans << endl;
