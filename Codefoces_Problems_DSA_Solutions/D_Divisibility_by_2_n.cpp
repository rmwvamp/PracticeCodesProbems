...
vector<int> DivisorsCount[int(2e5 + 1)];

void PreComputing_DivisorsofNumber()
{
    // doing it till 1e5
    for (int i = 1; i < 2e5 + 1; i++) // Putting factors/divisors at the number ith array position in the vector
    {
...
void solve()
{
    ...
    vector<ll> copiedarr(arr.begin(), arr.end());
    int arrcount = 0;
    for (int i = 0; i < n; i++)
    {
        while (copiedarr[i] % 2 == 0)
        {
            copiedarr[i] /= 2;
            arrcount++;
        }
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        numberof2.push_back(count);
    }
    sort(numberof2.begin(), numberof2.end(), greater<int>());
    int ans = 0;

    int operationcount = n - arrcount;
    if (operationcount <= 0)
    {
        cout << ans << endl;
        return;
    }
    for (int i = 0; i < numberof2.size(); i++)
    {
