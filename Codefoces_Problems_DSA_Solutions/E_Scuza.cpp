...
void solve()
{
    ...
    vector<ll> prefix(n);
    vector<ll> maxstairindex(n);

    vector<ll> question(q);
    // 1 2 1 5  -> 1 3 4 9     1 3 4 9
    // 1 2 2 5
    // 1 2 4 9 10->1
    ...
    for (int i = 0; i < n; i++)
    {
        maxstair = max(maxstair, arr[i]);
        if (i > 0)
            prefix[i] += prefix[i - 1];
        prefix[i] += arr[i];
        maxstairindex[i] = maxstair;
    }
    for (int i = 0; i < q; i++)
    {
        int idx = upper_bound(maxstairindex.begin(), maxstairindex.end(), question[i]) - maxstairindex.begin();
        if (idx - 1 >= 0)
            cout << prefix[idx - 1] << " ";
        else
            cout << "0" << " ";
