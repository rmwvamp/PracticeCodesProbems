        ...
        for (auto index : temp)
        {
            ...
            if (first == 1)
            {
                if (index.first == 0)
                {
                    ...
                    startindex = index.second;
                    thickness = max(thickness, index.second - index.first + 1);
                }
                else
                {
                    check = 0;
            ...
            else if (startindex + 1 == index.first)
            {
                startindex = index.second;
                thickness = max(thickness, index.second - index.first + 1);
            }
...
void rightsol()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        ll check = 1;
        ll lastindex = i;
        while (check && lastindex < n - 1)
        {
            ll j = lastindex + 1;
            ll sum = arr[j++];
            ll startidx = lastindex + 1;
            while (sum != currsum && currsum > sum)
            {
                sum += arr[j++];
            }
            if (sum != currsum)
            ...
            lastindex = j - 1;
            thickness = max(thickness, lastindex - startidx + 1);
        }
        if (check && lastindex == n - 1)
        {
            ans = min(ans, thickness);
        }
    }
