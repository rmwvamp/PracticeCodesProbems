...
vector<int> DivisorsCount[int(1e6 + 1)];
...
ll brutesolve(string &s)
{
    // SOLUTION STARTS

    ll n = s.size();
    // dbgm(DivisorsCount[36]);
    vector<bool> arr(n + 1);
    for (int i = 0; i < n; i++)
    {
        if (s[i] == '1')
    ...
    for (int i = 1; i <= n; i++)
    {
        if (arr[i] == true)
            continue;
        for (int j = 0; j < DivisorsCount[i].size(); j++)
        {
            ll st = 2;
            ll currnum = DivisorsCount[i][j];

            while (arr[currnum] == false && currnum < i)
                currnum = DivisorsCount[i][j] * st++;
            if (currnum == i)
            {
                cost += DivisorsCount[i][j];
                arr[i] = false;
                break;
            }
        }
...
ll optimisedsolve(string s)
{
    // SOLUTION STARTS

    ll n = s.size();
    // dbgm(DivisorsCount[1]);
    vector<bool> arr(n + 1);
    for (int i = 0; i < n; i++)
    {
        if (s[i] == '1')
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        for (int j = 0; j < DivisorsCount[i].size(); j++)
        {
            ...
            ll mul = i / DivisorsCount[i][j];

            if (mul == 1 || ((arr[(mul - 1) * DivisorsCount[i][j]] == false && DivisorsCount[i][j] > 1) || (DivisorsCount[i][j] == 1 && lastnumexist == -1)))
            {
                cost += DivisorsCount[i][j];
                arr[i] = false;
                break;
...
void solve()
{
    for (int i = 0; i < 10000; i++)
    {
        ...
        if (brutesolve(s) != optimisedsolve(s))
        {
            dbgm(n);
            dbgm(s);
            dbgm(brutesolve(s));
            dbgm(optimisedsolve(s));
        }
    }
}
int main()
