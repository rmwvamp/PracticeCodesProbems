...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (arr[i] == 1) // all zero after this-> remove, it would be 0.. all one before it

            maxchange = max(maxchange, -(prefix0[n - 1] - prefix0[i]) + prefix1[i] - 1);
        else // if it's zero, remove all one before it, it would be 1.. so all zero after it
            maxchange = max(maxchange, -prefix1[i] + (prefix0[n - 1] - prefix0[i]));
        // dbgm(maxchange);
    }
    cout << ans + maxchange << endl;
}
int main()
{
