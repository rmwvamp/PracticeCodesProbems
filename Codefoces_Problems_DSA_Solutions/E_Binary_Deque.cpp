...
void solve()
{
    ...
    {
        int currlo = -1, currhi = -1;
        int lo = -1, hi = -1;
        for (auto x : mmp)
        {
            if (mmp.find(abs(x.first - s)) != mmp.end())
            {
                for (auto v : mmp[x.first - s])
                {
                    if (x.first > v)
                    {
                        currlo = v;
...
void anothertry()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        int idx = upper_bound(prefix.begin(), prefix.end(), s + prefix[i - 1]) - prefix.begin() - 1;
        if (prefix[idx] + prefix[i - 1] == s)
        {
            int current_op = (i) + (n - 1 - idx);
            ans = min(ans, current_op);
        }
    }
    if (ans == INT_MAX)
        ans = -1;
