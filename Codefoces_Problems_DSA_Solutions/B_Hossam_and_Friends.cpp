...
void solve()
{
    ...
    scii(n, m);
    vector<ll> First_Not_Friend_On_Right(n + 1, n + 1);
    for (int i = 0; i < m; i++)
    {
        scii(l, r);
        if (l > r)
            swap(l, r);
        First_Not_Friend_On_Right[l] = min(r, First_Not_Friend_On_Right[l]);
    }
    ...
    {
        /* code */
        First_Not_Friend_On_Right[i] = min(First_Not_Friend_On_Right[i], First_Not_Friend_On_Right[i + 1]);
    }

    ll ans = 0;
    for (int i = 1; i < n + 1; i++)
    {
        ans += First_Not_Friend_On_Right[i] - i;
    }
    cout << ans << endl; 
}
int main()
