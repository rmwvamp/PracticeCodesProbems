...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ll hiidx = (upper_bound(arr.begin(), arr.end(), dif + arr[i]) - arr.begin()) - 1;
        ll lowidx = (lower_bound(arr.begin(), arr.end(), dif + arr[i]) - arr.begin());
        if (arr[hiidx] == dif + arr[i])
        {
            ans += hiidx - lowidx + 1;
            if (hiidx == i || lowidx == i || hiidx <= i || lowidx <= i)
                ans -= 1;
        }

        else
        {
            hiidx = (upper_bound(arr.begin(), arr.end(), arr[i] - dif) - arr.begin()) - 1;
            lowidx = (lower_bound(arr.begin(), arr.end(), arr[i] - dif) - arr.begin());
            if (arr[hiidx] == arr[i] - dif)
            {
...
void anothersolve()
{
    ...
    ll diff = mmp.end()->first - mmp.begin()->first;
    ll ans = (mmp.end()->second) * mmp.begin()->second;
    int flag = 1;
    for (auto x : mmp)
    {
        if (flag == 1)
        ...
        if (mmp[diff + x.first] && (diff + x.first) != x.first)
        {
            ans += x.second * mmp[diff + x.first];
        }
        else if (mmp[x.first - diff] && (x.first - diff) != x.first)
        {
            ans += x.second * mmp[x.first - diff];
        }
    }
    cout << ans << endl;
}
