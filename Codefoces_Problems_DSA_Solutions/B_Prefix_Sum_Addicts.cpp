...
void anothertry()
{
    ...
    // now we can find the n-k-2 elements using the prefix
    // for finding n-k-1 element.. we know it the prefix till that numebr..
    // we need to create the sum till there
    // now there are n-k-1 elements.. and we want to create minim

    arr.push_back(prefix[0] / (n - k + 1));
    if (prefix[0] % (n - k + 1) != 0 && prefix[0] > 0)
        arr[0]++; // 2
