...
void solve()
{
    ...
    for (int i = 0; i < n; i++) // array health->second
    {
        cin >> arr[i].second;
    }
    for (int i = 0; i < n; i++) // array power->first
    {
        cin >> arr[i].first;
    }
    sort(all(arr)); // O(n log(n))
...
{
    scii(n, k);
    priority_queue<pair<ll, ll>> h, p;
    vector<bool> died(n, false); // if true then died
    for (int i = 0; i < n; i++)
    {
        sci(x);
        h.push({-x, i});
    ...
    while (k > 0 && diedmonsters < n)
    {
        ...
        while (!p.empty() && died[p.top().second] == true)
            p.pop();
        if (!p.empty())
            k -= -p.top().first;
    }
