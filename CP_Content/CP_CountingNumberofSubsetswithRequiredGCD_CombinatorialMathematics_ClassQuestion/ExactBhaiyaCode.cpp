...
vector<vector<int>> DivisorsCount(1000);

void PreComputing_DivisorsofNumber()
{
    cout << "FrequencyCount: " << endl;
    // doing it till 1e5
    for (int i = 1; i < 1000 + 1; i++) // Putting factors/divisors at the number ith array position in the vector
    {
        for (int j = i; j < 1000 + 1; j += i)
        {
...
void solve()
{
    ...
    sci(gcdnumber);
    int FrequencyCount[int(1000 + 1)] = {0};
    dbg(m);
    // for (int i = 0; i < 30; i++)
    // {
    //     cout << "FrequencyCount: " << i << " " << FrequencyCount[i] << endl;
    // }
    for (auto MapIndices : m)
    {
        for (auto x : DivisorsCount[MapIndices.first])
        {
            FrequencyCount[x] += MapIndices.second;
        }
    }
    // for (int i = 0; i < 30; i++)
    // {
