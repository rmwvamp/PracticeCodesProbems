...
void solve()
{
    ...
    scii(n, l);
    vector<long long> LeftValues;
    vector<long long> RightValues;
    ...
    for (long long i = 1; i <= n; i++)
    {
        ...
        if (dir == 0)
        {
            LeftValues.push_back(arr[i - 1].first - 0);
        }
        else
        {
            RightValues.push_back(l - arr[i - 1].first);
        }
    }
    sort(all(arr));
    sort(all(LeftValues));

    sort(all(RightValues));
    for (long long i = 0; i < LeftValues.size(); i++)
    {
        arr[i].first = LeftValues[i];
    }
    for (long long j = 0; j < RightValues.size(); j++)
    {
        arr[n - 1 - j].first = RightValues[j];
    }
    long long j = n - 1;
    long long i = 0;
