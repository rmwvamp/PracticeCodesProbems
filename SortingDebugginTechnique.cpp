...
int BruteSolve(int n, int target, vector<int> arr)
{
    // sci(n);
    // vector<int> arr(n);
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++)
        {
            for (int k = j + 1; k < n; k++)
            {
                ...
                if (sum == target)
                {
                    // ans.first.first.push_back(arr[i]);
                    // ans.pb.first.first(arr[i]);
                    ans.pb({{arr[i], arr[j]}, arr[k]});
        ...
        {
            /* code */
            cout << ans[i].first.first << ", " << ans[i].first.second << " and " << ans[i].second << endl;
        }
    }
